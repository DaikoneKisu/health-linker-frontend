...
import { useRuralProfessional } from "../../../hooks/queries/admin";
import { Field, FieldProps, Form, Formik } from "formik";
import { useCommonToast } from "../../../hooks/useCommonToast";
import { useMutation } from "@tanstack/react-query";
import { updateRuralPassword } from "../../../api/admin";
...
export default function RuralesAdmin({ match }: Props) {
  const { data, isLoading } = useRuralProfessional(match.params.document);

  const updateMutation = useMutation({
    mutationFn: updateRuralPassword,
  });
            ...
            <div>
              <section className={`${styles.userSection} ion-padding`}>
                ...
                <div className={`${styles.userDiv}`}>
                  ...
                  <IonText>Centro de salud: {data.data.zone}</IonText>
                  <IonText>Cédula: {data.data.document}</IonText>
                  <IonText>Correo: {data.data.email}</IonText>
                  <IonText>Casos creados: {data.data.caseCount}</IonText>
                </div>
              </section>
